...
- `clients/desktop/` — Avalonia 11 + ReactiveUI Windows-клиент: 6 экранов + P2P sender (SIPSorcery 8.0.0 + SignalR.Client), FolderShareRegistry, ECDH+AES-GCM, online-toggle, DeviceIdentity (Phase 6.5), **двухуровневая проверка device-folder binding перед P2P** (2026-05-11); **Phase 9.5:** `Expander` + `SettingsView` (профиль + rename устройства); **Desktop Phase 8 (2026-05-15):** Semi.Avalonia удалена → `FluentTheme` + `AppStyles.axaml`; централизованная палитра в `App.axaml` (`BrandPrimaryBrush=#6938EF`, ink-шкала, status, radius); Inter Tight глобально; боковая панель-навигация (`ShellWindow` sidebar 220px); новые страницы `HomeView` (4 stat-карточки + recent 6), `LinksView` (GET /api/share-links + Copy/Revoke), `RecentView` (top 12 по lastAccessedAt); `FolderDetailView` — третья вкладка «Свойства» (ID, даты, ownerEmail, bound device + Отвязать); `SharedWithMeView` → OwnerEmail; `FolderResponse` + `OwnerEmail/LastAccessedAt`, `ShareLinkResponse` + `ShareUrl`, новый `ShareLinkOwnerResponse`, `ISharesApi.ListAllSharesAsync`; `IClipboardService/ClipboardService`; **43 desktop-тестов**
...
- **101 unit .NET + integration тестов + 43 Desktop-тестов + 35 Vitest тестов — все зелёные**
